...
// SaveConversation fires off a goroutine that updates the summary and
// embedding, then saves the conversation to disk. The `Conversation`'s
// `Summary` and `Modified` fields will be updated in place.
func (c *Conversation) SaveConversation() {
	// Prepare to save the conversation by creating a counting semaphore to
	// track the completion of the three save operations.
	wg := sync.WaitGroup{}
	wg.Add(3)

	go func() {
		// First, the synchronous tasks that must be executed in order.

		// Update the Summary and Modified time
		c.updateSummary()

		// Now that the summary has been updated, regenerate the embedding
		c.updateEmbedding()

		// Update the index file ($FNORD_HOME/conversations/index.jsonl)
		go func() {
			defer wg.Done()
			c.saveConversationIndexEntry()
		}()
...
// updateSummary updates Conversation.Summary by replacing it with a new
// summary of the conversation, generated by using GPT to summarize the
// conversation transcript. The `Modified` field is also updated to the current
// timestamp.
func (c *Conversation) updateSummary() error {
	c.Modified = time.Now()

	// Update summary
	summary, err := c.DataStore.gptClient.GetSummary(c.Messages)
	if err != nil {
		return err
	}
...
// updateEmbedding updates the conversation's embedding by replacing it with a
// new embedding generated from the current conversation summary. The `Hash`
// field is also updated to the hash of the current conversation summary. This
// method does NOT update the `Modified` field.
//
// Note that before calling this function, the `updateSummary` function should
// be called to ensure that the `Summary` and `Modified` fields are up to date.
func (c *Conversation) updateEmbedding() error {
	// Generate a new hash from the summary.
	hash := sha256.Sum256([]byte(c.Summary))

	// If the hash has not changed, there is no work to do.
	if c.Hash == hash {
		return nil
	}

	// Otherwise, generate an embedding for the summary text.
	embedding, err := c.DataStore.gptClient.GetEmbedding(c.Summary)
	if err != nil {
		return err
	}

	// Update the conversation with the updated hash.
	c.Hash = hash

	// Update the conversation with the updated embedding.
